...
const frontendConfig = {
  ...
  entry: {
    index: './src/index.js',
    droneTest: './DroneTest/DroneTest.js',
  },
  output: {
  ...
  plugins: [
    ...
    // This webpage is for TEST purpose
    new HtmlWebpackPlugin({
      template: './DroneTest/DroneTest.html',
      filename: 'DroneTest.html',
      chunks: ['droneTest']
    }),
    new CopyPlugin({
      patterns: [
...
const backendConfig = {
  ...
  entry: {
    ...
    droneControl: './server/DroneSocketControl.js',
  },
  output: {
    filename: '[name].js',
    path: path.resolve(__dirname, 'dist'),
